...
task 'data:export', [:zip_path] => :environment do |task, args|
  if args[:zip_path].blank?
    STDERR.puts "ERROR: rake data:export[<path to ZIP file>]"
    exit 1
  ...
  data = ZippedData.new(args[:zip_path], create: true)
...
task 'data:import', [:zip_path] => :environment do |task, args|
  if args[:zip_path].blank?
    STDERR.puts "ERROR: rake data:import[<path to ZIP file>]"
    exit 1
  elsif !File.exists?(args[:zip_path])
  ...
  data = ZippedData.new(args[:zip_path])
